...
public class WarehouseExport {
	...
	private static void export(HSSFWorkbook wb,Date date) throws SQLException{
        ...
        // 第六步，将文件存到指定位置  
        try{  
            FileOutputStream fout = new FileOutputStream("D:/总仓数据.xls");  
            wb.write(fout);  
            fout.close();  
        }catch (Exception e){  
            e.printStackTrace();  
