...
public class NewDayOrder {
	...
	public static void main(String[] args) throws ParseException {
		...
		String beginStr = "2015-12-16";
        String endStr = "2015-12-16";
	...
	private static void exprotWarehouseFunction(Integer warehouseId,String warehouseName,String dateStr){
		String sql = "SELECT torder.order_no AS \"订单号\", torder.serial_number AS \"交易号\", \"正常订单\" AS \"正常订单\", orderPay.pay_price / 100 AS \"实付金额\", orderPay.pay_mbean_price / 100 AS \"妈豆优惠\", orderPay.pay_coupon_price / 100 AS \"优惠券抵扣优惠\", orderPay.pay_freight_price / 100 AS \"运费\", orderPay.pay_shop_price / 100 AS \"门店金额\", CASE orderPay.pay_type_id WHEN 0 THEN \"支付宝V1.0\"WHEN 1 THEN \"银联支付\"WHEN 2 THEN \"微信支付\"WHEN 3 THEN \"妈豆支付\"WHEN 4 THEN \"支付宝V1.1\"WHEN 5 THEN \"微信公众号支付\"WHEN 6 THEN \"测试支付宝\"END AS \"支付方式\", orderPay.pay_account AS \"支付账户\", CASE torder.order_state WHEN 0 THEN \"已完成\"WHEN 4 THEN \"代发货\"WHEN 5 THEN \"待收货 \"WHEN 6 THEN \"待评价\"WHEN 9 THEN \"标记退款退货\"END AS \"订单状态\", torder.shop_name \"门店名称\", torder.warehouse_name \"仓库名称\", gb_div.DIV_NAME AS \"分公司名称\", torder.created_time \"下单时间\", torder.payment_time \"支付时间\", \"\" AS \"退款时间\", torder.tml_num_id AS \"好孩子单号\", CASE torder.relation_type WHEN 1 THEN \"一元购\"WHEN 3 THEN \"特卖会\"END AS \"标记\" ,CASE torder.platform_source WHEN 1 THEN \"APP订单\"WHEN 2 THEN \"云订单\"END AS \"订单来源\" FROM t_order torder LEFT JOIN t_order_pay orderPay ON torder.order_no = orderPay.order_no LEFT JOIN gb_div ON torder.company_id = gb_div.DIV_NUM_ID WHERE 1 = 1 AND DATE_FORMAT(torder.created_time, \"%Y-%m-%d\") = DATE_FORMAT(?, \"%Y-%m-%d\") AND torder.parent_order_no IS NOT NULL AND torder.order_state IN (4, 5, 6, 0, 9) AND torder.relation_type IS NULL AND torder.warehouse_id = ? UNION ALL SELECT refundLine.refund_line_no AS \"退款退货单号\", refundLine.refund_serial_number AS \"交易号\", \"退款订单\" AS \"正常订单\", - refundLine.refund_amount / 100 AS \"退款金额\", - orderLine.pay_mbean_price / 100 AS \"妈豆优惠\", - orderLine.pay_coupon_act_price / 100 AS \"优惠券抵扣优惠\", 0 AS \"运费\", - (refundLine.refund_amount + orderLine.pay_mbean_price + orderLine.pay_coupon_act_price ) / 100 AS \"门店退款\", CASE orderPay.pay_type_id WHEN 0 THEN \"支付宝V1.0\"WHEN 1 THEN \"银联支付\"WHEN 2 THEN \"微信支付\"WHEN 3 THEN \"妈豆支付\"WHEN 4 THEN \"支付宝V1.1\"WHEN 5 THEN \"微信公众号支付\"WHEN 6 THEN \"测试支付宝\"END AS \"退款方式\", refundLine.refund_account AS \"退款账户\", \"退款完成\" AS \"退款状态\", torder.shop_name \"门店名称\", torder.warehouse_name \"仓库名称\", gb_div.DIV_NAME AS \"分公司名称\", torder.created_time \"下单时间\", torder.payment_time \"支付时间\", refundLine.end_time AS \"退款时间\", torder.tml_num_id AS \"好孩子单号\", CASE refundLine.refund_type WHEN 1 THEN \"退货\"WHEN 2 THEN \"退款\"END AS \"标记\",CASE torder.platform_source WHEN 1 THEN \"APP订单\"WHEN 2 THEN \"云订单\"END AS \"订单来源\" FROM t_refund_line AS refundLine LEFT JOIN t_order_line AS orderLine ON refundLine.order_no = orderLine.order_no AND refundLine.order_line_id = orderLine.good_item_id LEFT JOIN t_order AS torder ON refundLine.order_no = torder.order_no LEFT JOIN t_order_pay orderPay ON torder.order_no = orderPay.order_no LEFT JOIN gb_div ON torder.company_id = gb_div.DIV_NUM_ID WHERE 1 = 1 AND refundLine. STATUS = 4 AND DATE_FORMAT(refundLine.end_time, \"%Y-%m-%d\") = DATE_FORMAT(?, \"%Y-%m-%d\") AND torder.relation_type IS NULL AND torder.warehouse_id = ?";
	...
	private static void companyExport(String dateStr){
		...
		warehouses.put(29, "合肥分公司");
		warehouses.put(44, "辽宁分公司");
		warehouses.put(47, "湖北分公司");
		warehouses.put(45, "苏州分公司");
		warehouses.put(23, "湖南分公司");
		warehouses.put(35, "江西分公司");
		warehouses.put(37, "昆明分公司");
		warehouses.put(34, "济南分公司");
		warehouses.put(41, "青岛分公司");
		warehouses.put(57, "重庆分公司");
		warehouses.put(26, "南宁分公司");
		warehouses.put(46, "天津分公司");
